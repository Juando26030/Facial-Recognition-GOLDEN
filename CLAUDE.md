...
**Documentos de producto (backlog, visión, modelo de datos):** el usuario los referencia como `01_VISION_Y_ALCANCE.md` a `05_MODELO_DATOS.md`, pero **no viven en este repo** (2026-09-19, confirmado — no aparecen en ningún commit). Se gestionan en otra conversación/lugar aparte. Si una tarea los referencia y hacen falta, pedírselos al usuario en vez de asumir su contenido.
...
**Carga de base de asistentes, agnóstica al método (2026-09-19, Épico 1/Historia 1.1):** `POST /api/bulk_register` (mismo endpoint de siempre, comportamiento extendido) ya NO exige el `.zip` de fotos — `zip_file` es opcional. Sin zip: solo crea/actualiza identidad (`User`) y asocia a `EventAttendee`, suficiente para acreditar por cédula. Con zip: además procesa las fotos a encoding biométrico, igual que antes (necesario para que Facial reconozca a esas personas). Los errores de fila ya no abortan la carga completa (antes había un `except: pass` silencioso) — se acumulan en `errors: []` en la respuesta y se listan en la consola del navegador; el mensaje de éxito indica cuántas filas tuvieron problema.
...
- `EventAttendee(id, event_id, user_id, tenant_id, created_at)` — **decisión de modelado (2026-09-19, ver `05_MODELO_DATOS.md` §3 pregunta 1, resuelta con opción (b)):** la lista de personas esperadas/asociadas a UN evento puntual, separada de `User`. Se eligió esta opción (en vez de reusar `User` directamente como "la lista del evento") porque el pedido explícito fue que "cargar la base de un evento sirva para cualquier tipo de registro, no una base por cada tipo" — `EventAttendee` es agnóstico al método (cédula, facial, QR futuro); solo `User.face_encoding` es específico de facial. Una fila aquí significa "esta persona se espera/se reconoce para este evento", independiente de si ya se presentó (eso lo dice `AccessLog`, filtrado por `event_id`). Se crea al cargar el roster (`POST /api/events/{id}/roster`, antes `bulk_register`) y también se crea sobre la marcha (upsert) cuando alguien se registra/reconoce en un evento sin haber estado precargado — así el directorio de ESE evento siempre incluye a todos los que efectivamente participaron, se hayan precargado o no.
