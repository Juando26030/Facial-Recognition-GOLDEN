...
**Usuario `cliente` (2026-09-16):** solo lo crean `admin`/`super_admin` (no coordinador) desde `/admin/staff`, y se autoriza a un evento igual que un digitador (`POST /staff/{id}/authorize-event/{id}`, admin+). Al loguearse con exactamente un evento activo autorizado entra derecho a `/kiosk/{event_id}` igual que un digitador, pero ahí solo ve dos pestañas: **Estadísticas** (placeholder, todavía sin diseñar/mockeado en `templates/kiosk.html`) y **Directorio en Vivo**. No ve el escáner, registro, carga masiva, reporte ni usuarios temporales.
...
- **Pendiente: selector de hora tipo reloj analógico.** El usuario pidió explícitamente que la selección de hora en el formulario de evento NO sea el spinner nativo del navegador (`<input type="time">`, lo que hay hoy) sino un widget visual de reloj (como el de apps móviles). Es un componente custom (SVG/canvas + JS), no hay nada nativo en HTML que lo dé — no se construyó todavía, sigue siendo `<input type="time">` por ahora.
- **Pendiente: ciudades dependientes del país seleccionado.** El usuario pidió que al elegir un país en el formulario de evento, el campo Ciudad se llene con TODAS las ciudades de ese país (no solo las principales), tomadas de "bases de datos de internet", manteniendo la opción de escribir manualmente si no está en la lista. Esto requiere una decisión de datos que no se tomó todavía: un dataset embebido (ej. GeoNames, pesado — cientos de miles de filas) vs. una API externa de geocodificación en tiempo real (necesita llave/cuota). Por ahora Ciudad sigue siendo texto libre sin sugerencias.
