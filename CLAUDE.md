...
app/
  ...
  cities_data.py    Dataset curado país→ciudades para el selector del formulario de evento
...
- ~~Selector de hora tipo reloj~~ **Resuelto (2026-09-17):** `static/js/clockpicker.js`, widget custom (SVG, sin dependencias externas) — reemplaza el `<input type="time">` nativo en los 4 campos de hora del formulario de evento. `attachClockPicker(input)` sobre un `<input type="text" readonly>`; ver el archivo para cómo reusarlo en otro formulario.
- **Ciudades por país: resuelto parcialmente (2026-09-17).** `app/cities_data.py` tiene una lista **curada** (no exhaustiva) de ciudades principales por país, servida vía `GET /api/cities?country=X`, y el campo Ciudad del formulario de evento se auto-llena con sugerencias (`<datalist>`) según el país elegido — sigue aceptando texto libre para lo que no esté en la lista. Cubrir literalmente TODAS las ciudades del mundo necesitaría un dataset externo real (GeoNames u otra fuente, cientos de miles de filas) o una API de geocodificación con llave/cuota — no se integró, es la siguiente mejora si hace falta más cobertura.
