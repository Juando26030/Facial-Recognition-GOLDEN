...
- **Dos métodos reales hoy: Facial (`GET /kiosk/{event_id}/facial`) y Cédula (`GET /kiosk/{event_id}/cedula`, agregado 2026-09-19, Épico 1/Sprint 1 del backlog de producto)**, cada uno con su propio template (`kiosk.html` / `kiosk_cedula.html`) compartiendo `main.py: _resolve_kiosk_page` para el boilerplate de acceso. Ambos escriben `AccessLog` con el mismo `event_id`, así que Directorio en Vivo y Reporte no distinguen por método. **QR** sigue como tarjeta deshabilitada ("Próximamente") en `kiosk_select.html` — placeholder a propósito.
- **Cédula (`kiosk_cedula.html` + `POST /api/checkin-cedula`):** un solo `<input>` autoenfocado que interpreta la lectura del lector de código de barras como tecleo + Enter (sin submit manual) — dispara la búsqueda automáticamente. Mismo shape de respuesta que `/api/recognize` (`result: "SÍ"/"NO"` + `data`), a propósito, para reusar el mismo patrón de UI. Si la cédula no existe en el tenant, muestra un mini-formulario de alta manual que reusa `POST /api/register` (sin foto, ver abajo). Gateada igual que facial: `require_role("digitador")` + `require_event_in_progress` (bloqueado si el evento no está `en_proceso`, con el mismo aviso amarillo e input deshabilitado que ya usa `kiosk.html`).
