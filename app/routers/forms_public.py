...
def _payload_form(db: Session, form: WebForm, claims: dict) -> dict:
    ...
    return {"design": formlib.public_design(design), "id_docs": formlib.id_docs_public(), "quota": formsvc.quota_public(db, form), "prefill": prefill, "readonly": readonly, "badge_email_field": formsvc.badge_email_field(design) if formsvc.wants_digital_badge(db, form) else None, "capacity_left": None if form.capacity is None else max(0, form.capacity - formsvc.held_count(db, form))}
